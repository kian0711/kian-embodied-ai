...
type ReadingPaper = { id:number; title:string; innovation:string; method:string; result:string; implementation:string; pdfName:string; pdfSize:number; createdAt:string };
...
export function MyPaperLibrary() {
  ...
  return <section className="my-papers shell" id="my-papers">
    ...
    {loading ? <div className="reading-empty">正在读取精读档案…</div> : papers.length === 0 ? <div className="reading-empty"><b>第一篇精读正在准备中</b><span>标题、创新点、方法、结果、实现途径与完整 PDF 将展示在这里。</span></div> :
      <div className="reading-list">{papers.map((paper, index) => <article className="reading-card" key={paper.id}>
        <div className="reading-card-head"><span>{String(index + 1).padStart(2,"0")}</span><div><small>{new Date(paper.createdAt).toLocaleDateString("zh-CN")} · KIAN 精读</small><h3>{paper.title}</h3></div><a href={`/api/reading-papers/${paper.id}/pdf`} target="_blank" rel="noreferrer">阅读 PDF ↗</a></div>
        ...
        <footer><span>PDF · {(paper.pdfSize / 1024 / 1024).toFixed(1)} MB</span><span>{paper.pdfName}</span></footer>
      </article>)}</div>}
  </section>;
}
