  ...
  return <section className="my-papers shell" id="my-papers">
    ...
    {loading ? <div className="reading-empty">正在读取精读档案…</div> : papers.length === 0 ? <div className="reading-empty"><b>第一篇精读正在准备中</b><span>标题、创新点、方法、结果、实现途径与完整 PDF 将展示在这里。</span></div> :
      <div className="reading-list">{papers.map((paper, index) => <article className="reading-card" key={paper.id}>
        <div className="reading-card-head"><span>{String(index + 1).padStart(2,"0")}</span><div><small>{new Date(paper.createdAt).toLocaleDateString("zh-CN")} · KIAN 精读</small><h3>{paper.title}</h3></div><a href={paper.paperUrl} target="_blank" rel="noreferrer">打开论文 ↗</a></div>
        ...
        <footer><span>论文原文链接</span><span>{paper.paperUrl}</span></footer>
      </article>)}</div>}
  </section>;
}
