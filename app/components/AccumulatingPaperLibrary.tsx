...
export function AccumulatingPaperLibrary() {
  ...
  return <>
    ...
    <div className={`archive-list ${loading?"loading":""}`}>{data.papers.map((paper,index)=><article className="archive-paper" key={paper.id}>
      <div className="archive-index">{String((data.page-1)*8+index+1).padStart(2,"0")}</div><div className="archive-year">{paper.year}<small>{paper.categoryLabel}</small></div><div className="archive-main"><h3>{paper.title}</h3><p>{paper.abstract||"暂无摘要，点击原文查看完整内容。"}</p><span>{paper.authors}</span></div><div className="archive-metrics"><span>引用 {paper.citations}</span><small>收录 {paper.addedAt}</small></div><div className="archive-actions"><a href={paper.url} target="_blank" rel="noreferrer">阅读原文 ↗</a><a className="translate" href={translate(paper)} target="_blank" rel="noreferrer">翻译摘要 文</a><a className="fallback" href={fallback(paper)} target="_blank" rel="noreferrer">备用搜索 ⌕</a></div>
    </article>)}</div>
