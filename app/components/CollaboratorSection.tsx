...
type Collaborator = { id:number; name:string; school:string; researchDirection:string; bio:string };
...
export function CollaboratorSection() {
  ...
  return <section className="collaboration-section" id="collaboration"><div className="shell">
    ...
    {members.length ? <div className="collaborator-grid">{members.map((member, index) => <article className="collaborator-card" key={member.id}>
      <div className="collaborator-photo"><img src={`/api/collaborators/${member.id}/photo`} alt={`${member.name}的个人照片`} /><span>{String(index + 1).padStart(2,"0")}</span></div>
      <div className="collaborator-info"><small>{member.school}</small><h3>{member.name}</h3><p className="research-label">研究方向</p><strong>{member.researchDirection}</strong><p className="collaborator-bio">{member.bio}</p></div>
