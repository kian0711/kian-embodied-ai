...
type Collaborator = { id:number; name:string; school:string; researchDirection:string; bio:string; photoPosition?:string; photoUpdatedAt?:string };
...
export function CollaboratorSection() {
  ...
  return <section className="collaboration-section" id="collaboration"><div className="shell">
    ...
    {members.length ? <div className="collaborator-grid">{members.map((member, index) => <article className="collaborator-card" key={member.id}>
      <div className="collaborator-photo"><img src={`/api/collaborators/${member.id}/photo?v=${encodeURIComponent(member.photoUpdatedAt||"")}`} alt={`${member.name}的个人照片`} style={{objectPosition:member.photoPosition || "50% 20%"}} /><span>{String(index + 1).padStart(2,"0")}</span></div>
