...
export async function PUT(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const db=getD1(); const previous=await db.prepare("SELECT photo_key AS photoKey, photo_name AS photoName FROM collaborators WHERE id = ?").bind(Number(id)).first<{photoKey:string;photoName:string}>();
    ...
    try{await db.prepare("UPDATE collaborators SET name = ?, school = ?, research_direction = ?, bio = ?, photo_key = ?, photo_name = ?, photo_position = ? WHERE id = ?").bind(name,school,researchDirection,bio,photoKey,photoName,photoPosition,Number(id)).run();}
