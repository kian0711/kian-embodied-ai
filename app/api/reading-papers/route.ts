...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const values = Object.fromEntries(fields.map((field) => [field, String(form[field] || "").trim()])) as Record<typeof fields[number], string>;
    ...
    const paperUrl = String(form.paperUrl || "").trim();
    ...
    const inserted = await db.prepare(`INSERT INTO reading_papers
      (title,innovation,method,result,implementation,paper_url,pdf_key,pdf_name,pdf_size,created_at)
      VALUES (?,?,?,?,?,?,?,?,?,?) RETURNING id`).bind(values.title, values.innovation, values.method, values.result, values.implementation, paperUrl, "", "", 0, createdAt).first<{ id: number }>();
