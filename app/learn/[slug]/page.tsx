...
export default async function LearningPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className={`learning-page ${track.color}`}>
    ...
    <section className="learn-block shell" id="chapters"><div className="learn-section-head"><div><p className="eyebrow"><span /> COURSE MODULES</p><h2>学习路线</h2></div><p>可展开速览，也可以进入 2000+ 字完整课程。</p></div><div className="chapter-list">{track.chapters.map((chapter) => { const detail = getModuleDetail(track.id, chapter.no); return <details key={chapter.no} className="course-module"><summary><span>{chapter.no}</span><div><small>MODULE {chapter.no}</small><h3>{chapter.title}</h3><p>{chapter.desc}</p></div><ol>{chapter.lessons.map((lesson) => <li key={lesson}>{lesson}</li>)}</ol><b className="module-toggle">＋</b></summary>{detail && <div className="module-body"><section><small>01 · 核心理论</small><h4>先理解为什么</h4><p>{detail.theory}</p></section><section className="case-card"><small>02 · 案例拆解</small><h4>{detail.caseStudy.title}</h4><p>{detail.caseStudy.story}</p></section><section><small>03 · 具体应用</small><h4>它能用在哪里</h4><ul>{detail.applications.map((item)=><li key={item}>{item}</li>)}</ul></section><section className="practice-card"><small>04 · 动手练习</small><h4>{detail.practice}</h4><p className="checkpoint">验收标准：{chapter.checkpoint}</p><a className="full-lesson-link" href={`/learn/${track.id}/${chapter.no}`}>进入完整课程 →</a></section></div>}</details> })}</div></section>
