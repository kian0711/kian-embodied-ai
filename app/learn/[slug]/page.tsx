...
export default async function LearningPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className={`learning-page ${track.color}`}>
    ...
    <section className="learn-block shell" id="chapters"><div className="learn-section-head"><div><p className="eyebrow"><span /> COURSE MODULES</p><h2>学习路线</h2></div><p>按顺序完成，每一章都有可验证的学习检查点。</p></div><div className="chapter-list">{track.chapters.map((chapter) => <article key={chapter.no}><span>{chapter.no}</span><div><small>MODULE {chapter.no}</small><h3>{chapter.title}</h3><p>{chapter.desc}</p><em className="checkpoint">验收：{chapter.checkpoint}</em></div><ol>{chapter.lessons.map((lesson) => <li key={lesson}>{lesson}</li>)}</ol><a href={`#project-${track.id}`} aria-label={`查看${chapter.title}项目验收`}>↗</a></article>)}</div></section>
